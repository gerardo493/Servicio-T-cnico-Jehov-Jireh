...
def obtener_estadisticas():
    ...
    for factura_id, factura in facturas.items():
        ...
        if cliente_id and cliente_id in clientes:
            factura_copia['cliente'] = clientes[cliente_id].get('nombre', 'Cliente no encontrado')
        else:
            factura_copia['cliente'] = 'Cliente no encontrado'
...
def mostrar_notas_entrega():
    """Listado de notas de entrega con filtros, ordenamiento, totales y exportación CSV."""
    notas = cargar_datos(ARCHIVO_NOTAS_ENTREGA)
    clientes = cargar_datos(ARCHIVO_CLIENTES)

    # Normalizar/calcular totales y estados derivados
    for id, nota in list(notas.items()):
        try:
            ...
            tasa_bcv = float(nota.get('tasa_bcv', 1.0) or 1.0)
            descuento_total = float(nota.get('descuento_total', 0) or 0)
            iva_total = float(nota.get('iva_total', 0) or 0)
            total_usd = float(nota.get('total_usd') or (subtotal_usd - descuento_total + iva_total) or 0.0)
            total_bs = float(nota.get('total_bs') or (total_usd * tasa_bcv))
            pagos = nota.get('pagos', []) or []
            total_abonado = 0.0
            for p in pagos:
                try:
                    monto = p.get('monto', 0)
            ...
            print(f"Error normalizando nota {id}: {e}")
    ...
    rows = []  # (id, nota)
    for id, f in notas.items():
        # Validar que el ID sea válido
        if not id or str(id).strip() == '':
            print(f"ADVERTENCIA: Nota de entrega con ID inválido encontrada: {id}")
            continue
    ...
    if request.args.get('export') == 'csv':
        ...
        response.headers['Content-Disposition'] = 'attachment; filename=notas_entrega.csv'
...
# Ruta de respaldo para facturas (redirige a notas de entrega)
@app.route('/facturas')
@login_required
def mostrar_facturas():
    """Redirige a la vista de notas de entrega para mantener compatibilidad."""
...
def editar_factura(id):
    ...
    if request.method == 'POST':
        try:
            ...
            cantidades_antiguas = {}
            if 'cantidades' in factura and 'productos' in factura:
                cantidades_antiguas = dict(zip(factura['productos'], factura['cantidades']))
            elif 'productos' in factura:
                # Para facturas del servicio técnico, extraer cantidades de los productos
                for producto in factura['productos']:
                    if isinstance(producto, dict) and 'id' in producto:
                        cantidades_antiguas[producto['id']] = producto.get('cantidad', 1)
            ...
            # Solo asignar cantidades si no es una factura del servicio técnico
            if 'orden_servicio_id' not in factura:
...
@app.route('/facturas/<id>/convertir-a-nota-entrega', methods=['POST'])
@login_required
def convertir_factura_a_nota_entrega(id):
    """Convierte una factura a nota de entrega."""
    try:
        facturas = cargar_datos(ARCHIVO_FACTURAS)
        notas = cargar_datos(ARCHIVO_NOTAS_ENTREGA)
        ...
        if factura.get('estado') == 'PAGADA' or factura.get('estado') == 'cobrada':
            flash('No se puede convertir una factura pagada a nota de entrega', 'warning')
            return redirect(url_for('mostrar_facturas'))
        
        # Obtener numeración para nota de entrega
        usuario_actual = session.get('usuario', 'SISTEMA')
        numero_fiscal, numero_secuencial = control_numeracion.obtener_siguiente_numero('NOTA_ENTREGA', usuario_actual)
        
        # Crear nota de entrega basada en la factura
        nota_entrega = {
            ...
            'descuento_total': factura.get('descuento_total', 0),
            'iva': 0,  # Las notas de entrega no tienen IVA
            'iva_total': 0,
            'total_usd': factura.get('subtotal_usd', 0),  # Sin IVA
            'modalidad_pago': 'credito',  # Por defecto a crédito
            'dias_credito': factura.get('dias_credito', 30),
            'fecha_vencimiento': (datetime.now() + timedelta(days=factura.get('dias_credito', 30))).strftime('%Y-%m-%d'),
            'factura_origen': id,  # Referencia a la factura original
            'estado': 'PENDIENTE',
            'pagos': [],
            'tasa_bcv': factura.get('tasa_bcv', 36.00),
            'observaciones': f'Convertida desde factura #{id}'
        }
        
        # Guardar nota de entrega
        notas[nota_entrega['numero']] = nota_entrega
        guardar_datos(ARCHIVO_NOTAS_ENTREGA, notas)
        
        # Marcar factura como convertida a nota
        factura['estado'] = 'CONVERTIDA_A_NOTA'
        factura['nota_entrega_generada'] = nota_entrega['numero']
        guardar_datos(ARCHIVO_FACTURAS, facturas)
        
        # Registrar en bitácora
        registrar_bitacora(session['usuario'], 'Convertir factura a nota de entrega', f"Factura: {id} -> Nota: {nota_entrega['numero']}")
        
        flash(f'Factura #{id} convertida a nota de entrega #{nota_entrega["numero"]}', 'success')
        return redirect(url_for('mostrar_notas_entrega'))
        
    except Exception as e:
        flash(f'Error al convertir factura a nota de entrega: {str(e)}', 'danger')
        return redirect(url_for('mostrar_facturas'))
...
def api_pagos_filtrados():
    ...
    try:
        ...
        facturas_data = cargar_datos(ARCHIVO_FACTURAS)
        print(f"Tipo de datos cargados: {type(facturas_data)}")
        
        # Convertir diccionario a lista si es necesario
        if isinstance(facturas_data, dict):
            facturas = list(facturas_data.values())
        else:
        ...
        for factura in facturas:
            # Verificar si la factura está pagada (estado en minúscula)
            if factura.get('estado', '').lower() == 'pagada' and factura.get('pagos'):
                try:
                    # Obtener la fecha del último pago
                    ultimo_pago = factura['pagos'][-1]  # Último pago
                    ...
                    if fecha_pago_str:
                        ...
                        if incluir:
                            monto_usd = float(factura.get('total_usd', 0))
                            monto_bs = float(factura.get('total_bs', 0))
                            total_usd += monto_usd
                            total_bs += monto_bs
                            pagos.append(factura)
                            print(f"Incluido: ID {factura.get('id')}, Fecha: {fecha_pago}, USD: {monto_usd}, Bs: {monto_bs}")
                            
                except (ValueError, KeyError, IndexError) as e:
                    print(f"Error procesando factura {factura.get('id', 'N/A')}: {e}")
                    continue
...
def api_cobranza_filtrada():
    ...
    try:
        ...
        for factura in facturas:
            # Verificar si la factura está pendiente de pago
            if factura.get('estado', '').lower() in ['pendiente', 'por_cobrar', 'credito']:
                try:
                    # Usar fecha de la factura para el filtro
                    ...
                    if fecha_factura_str:
                        ...
                        incluir = False
                        if periodo == 'hoy':
                            incluir = fecha_factura == hoy
                        elif periodo == 'semana':
                            inicio_semana = hoy - timedelta(days=hoy.weekday())
                            incluir = fecha_factura >= inicio_semana and fecha_factura <= hoy
                        elif periodo == 'mes':
                            inicio_mes = hoy.replace(day=1)
                            incluir = fecha_factura >= inicio_mes and fecha_factura <= hoy
                        elif periodo == 'todos':
                            incluir = True
                        ...
                        if incluir:
                            ...
                            total_usd += monto_usd
                            total_bs += monto_bs
                            cobranza.append(factura)
                            print(f"Incluido: ID {factura.get('id')}, Fecha: {fecha_factura}, USD: {monto_usd}, Bs: {monto_bs}")
        ...
        return jsonify({
            ...
            'total_bs': total_bs_formatted,
            'periodo': periodo,
            'cantidad_facturas': len(cobranza)
        })
...
@app.route('/api/facturado-filtrado')
@login_required
def api_facturado_filtrado():
    """API para obtener facturado filtrado por período"""
    try:
        periodo = request.args.get('periodo', 'todos')
        print(f"Filtrando facturado para período: {periodo}")
        ...
        for factura in facturas:
            # Incluir todas las facturas (pagadas y pendientes)
            try:
                fecha_factura_str = factura.get('fecha', '')
        ...
        print(f"Facturado encontrado: {len(facturado)}, Total USD: {total_usd}, Total Bs: {total_bs}")
        ...
        return jsonify({
            ...
            'total_bs': total_bs_formatted,
            'periodo': periodo,
            'cantidad_facturas': len(facturado)
        })
        
    except Exception as e:
        print(f"Error en API facturado-filtrado: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e),
            'total_usd': '0,00',
...
def servicio_tecnico():
    ...
    try:
        ...
        ordenes_pendientes = len([o for o in ordenes.values() if o['estado'] in ['en_espera_revision', 'en_diagnostico', 'presupuesto_enviado', 'aprobado_por_cliente', 'en_reparacion']])
        ordenes_completadas = len([o for o in ordenes.values() if o['estado'] == 'entregado'])
        ...
        return render_template('servicio_tecnico/index.html', ordenes={}, config={})
...
def nueva_orden_servicio():
    ...
    if request.method == 'POST':
        try:
            ...
            ordenes = cargar_datos('ordenes_servicio.json')
            numero_orden = f"OS-{len(ordenes) + 1:08d}"
            ...
            nueva_orden = {
                ...
                "equipo": {
                    ...
                    "imei": request.form.get('equipo_imei'),
                    "color": request.form.get('equipo_color'),
                    "numero_serie": request.form.get('equipo_numero_serie')
                },
                "problema_reportado": request.form.get('problema_reportado'),
                "accesorios_entregados": {
                    "cargador": 'cargador' in request.form,
                    "audifonos": 'audifonos' in request.form,
                    "caja": 'caja' in request.form,
                    "cable": 'cable' in request.form,
                    "otros": request.form.get('accesorios_otros', '')
                },
                "condicion_fisica": {
                    "rayones": request.form.get('condicion_rayones', ''),
                    "golpes": request.form.get('condicion_golpes', ''),
                    "agua": 'agua' in request.form,
                    "observaciones": request.form.get('condicion_observaciones', '')
                },
                "diagnostico": {
            ...
            # Guardar orden
            ordenes[numero_orden] = nueva_orden
            guardar_datos('ordenes_servicio.json', ordenes)
            
            flash(f'Orden de servicio {numero_orden} creada exitosamente', 'success')
            return redirect(url_for('ver_orden_servicio', id=numero_orden))
            
        except Exception as e:
            flash(f'Error creando orden de servicio: {str(e)}', 'danger')
...
def actualizar_estado_orden(id):
    ...
    try:
        ...
        flash(f'Estado actualizado a: {config["estados_servicio"][nuevo_estado]["nombre"]}', 'success')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            ordenes[id]['diagnostico'] = {
                "problema_identificado": request.form.get('problema_identificado'),
                "solucion_aplicada": request.form.get('solucion_aplicada'),
                "piezas_usadas": request.form.getlist('piezas_usadas'),
                "comentarios_tecnicos": request.form.get('comentarios_tecnicos'),
                "costo_mano_obra": float(request.form.get('costo_mano_obra', 0)),
                "costo_piezas": float(request.form.get('costo_piezas', 0)),
                "total_estimado": float(request.form.get('total_estimado', 0))
            }
            
            # Actualizar estado a presupuesto enviado
            ordenes[id]['estado'] = 'presupuesto_enviado'
            ordenes[id]['fecha_actualizacion'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            ordenes[id]['historial_estados'].append({
                ...
                "fecha": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                "usuario": session.get('usuario', 'admin'),
                "comentarios": "Diagnóstico completado y presupuesto enviado"
            })
...
def generar_factura_servicio(id):
    """Generar factura automáticamente desde una orden de servicio"""
    try:
        ordenes = cargar_datos('ordenes_servicio.json')
        facturas = cargar_datos(ARCHIVO_FACTURAS)
        clientes = cargar_datos(ARCHIVO_CLIENTES)
        ...
        if orden['estado'] not in ['reparado', 'entregado']:
            flash('La orden debe estar reparada o entregada para generar la factura', 'warning')
            return redirect(url_for('ver_orden_servicio', id=id))
        
        # Generar número de factura
        numero_factura = f"FAC-{len(facturas) + 1:08d}"
        ...
        # Crear productos/servicios para la factura
        productos_factura = []
        cantidades = []
        precios = []
        ...
        productos_factura.append({
            "id": f"servicio_{id}",
            "nombre": f"Reparación {orden['equipo']['marca']} {orden['equipo']['modelo']}",
            "descripcion": f"Servicio técnico - {orden['diagnostico']['solucion_aplicada']}",
            "precio": orden['diagnostico']['costo_mano_obra'],
            "cantidad": 1,
            "subtotal": orden['diagnostico']['costo_mano_obra'],
            "subtotal_usd": orden['diagnostico']['costo_mano_obra']
        })
        cantidades.append("1")
        precios.append(str(orden['diagnostico']['costo_mano_obra']))
        
        # Agregar piezas utilizadas
        for pieza_id in orden['diagnostico']['piezas_usadas']:
            if pieza_id in inventario:
                pieza = inventario[pieza_id]
                productos_factura.append({
                    "id": pieza_id,
                    "nombre": pieza['nombre'],
                    ...
                    "subtotal_usd": pieza['precio']
                })
                cantidades.append("1")
                precios.append(str(pieza['precio']))
        
        # Obtener tasa BCV actual
        try:
            with open('ultima_tasa_bcv.json', 'r') as f:
                tasa_data = json.load(f)
                tasa_bcv = tasa_data.get('tasa', 36.0)
        except:
            tasa_bcv = 36.0
        ...
        nueva_factura = {
            ...
            "numero": numero_factura,  # Campo adicional para compatibilidad
            "fecha": datetime.now().strftime('%Y-%m-%d'),
            "cliente_id": cliente_id,
            "cliente": orden['cliente'],
            "productos": productos_factura,
            "cantidades": cantidades,  # Para compatibilidad con funciones de edición
            "precios": precios,        # Para compatibilidad con funciones de edición
            "subtotal": total_usd,
            "subtotal_usd": total_usd,
            "iva": 0,  # Configurar según necesidad
            "total": total_usd,
            "total_usd": total_usd,
            "total_bs": total_bs,
            "tasa_bcv": tasa_bcv,
            ...
            "observaciones": f"Factura generada desde orden de servicio {orden['numero_orden']}",
            "orden_servicio_id": id,
            "fecha_creacion": datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }
        
        # Guardar factura
        facturas[numero_factura] = nueva_factura
        guardar_datos(ARCHIVO_FACTURAS, facturas)
        
        # Actualizar orden de servicio
        ordenes[id]['factura_generada'] = numero_factura
        ordenes[id]['fecha_actualizacion'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        guardar_datos('ordenes_servicio.json', ordenes)
        
        flash(f'Factura {numero_factura} generada exitosamente desde la orden de servicio', 'success')
        return redirect(url_for('ver_factura', id=numero_factura))
        
    except Exception as e:
        flash(f'Error generando factura: {str(e)}', 'danger')
        return redirect(url_for('ver_orden_servicio', id=id))

@app.route('/servicio-tecnico/orden/<id>/presupuesto', methods=['GET'])
@login_required
def presupuesto_servicio(id):
    """Generar presupuesto para una orden de servicio"""
    try:
        ordenes = cargar_datos('ordenes_servicio.json')
        config = cargar_datos('config_servicio_tecnico.json')
        empresa = cargar_datos('empresa.json')
        ...
        # Mostrar presupuesto en HTML (para impresión)
        ...
        flash(f'Error generando presupuesto: {str(e)}', 'danger')
        return redirect(url_for('ver_orden_servicio', id=id))
...
def enviar_notificacion_servicio(id):
    ...
    try:
        ...
        if orden['diagnostico']['total_estimado'] > 0:
            mensaje += f"\n\nCosto estimado: ${orden['diagnostico']['total_estimado']:.2f} USD"
        
        # Enviar WhatsApp si está habilitado
        if config['configuracion_notificaciones']['whatsapp_habilitado']:
            try:
                # Usar la función de WhatsApp existente
                resultado_whatsapp = enviar_whatsapp_cliente(
                    orden['cliente']['telefono'], 
                    mensaje
                )
                if resultado_whatsapp.get('success'):
                    flash('Notificación enviada por WhatsApp exitosamente', 'success')
                else:
                    flash(f'Error enviando WhatsApp: {resultado_whatsapp.get("error", "Error desconocido")}', 'warning')
            except Exception as e:
                flash(f'Error enviando WhatsApp: {str(e)}', 'warning')
...
@app.route('/servicio-tecnico/migrar-facturas')
@login_required
def migrar_facturas_servicio():
    """Migrar facturas del servicio técnico para incluir campos faltantes"""
    try:
        facturas = cargar_datos(ARCHIVO_FACTURAS)
        facturas_migradas = 0
        
        for factura_id, factura in facturas.items():
            # Verificar si es una factura del servicio técnico
            if 'orden_servicio_id' in factura:
                # Agregar campos faltantes si no existen
                if 'total_usd' not in factura:
                    factura['total_usd'] = factura.get('total', 0)
                    facturas_migradas += 1
                
                if 'subtotal_usd' not in factura:
                    factura['subtotal_usd'] = factura.get('subtotal', 0)
                
                if 'total_bs' not in factura:
                    tasa_bcv = factura.get('tasa_bcv', 36.0)
                    factura['total_bs'] = factura['total_usd'] * tasa_bcv
                
                if 'numero' not in factura:
                    factura['numero'] = factura.get('numero_factura', factura_id)
                
                # Migrar productos si no tienen subtotal_usd
                if 'productos' in factura:
                    for producto in factura['productos']:
                        if isinstance(producto, dict) and 'subtotal_usd' not in producto:
                            producto['subtotal_usd'] = producto.get('subtotal', 0)
                
                # Agregar campos cantidades y precios para compatibilidad
                if 'cantidades' not in factura and 'productos' in factura:
                    cantidades = []
                    precios = []
                    for producto in factura['productos']:
                        if isinstance(producto, dict):
                            cantidades.append(str(producto.get('cantidad', 1)))
                            precios.append(str(producto.get('precio', 0)))
                        else:
                            # Si producto no es un diccionario, usar valores por defecto
                            cantidades.append('1')
                            precios.append('0')
                    factura['cantidades'] = cantidades
                    factura['precios'] = precios
        ...
        if facturas_migradas > 0:
            guardar_datos(ARCHIVO_FACTURAS, facturas)
            flash(f'Se migraron {facturas_migradas} facturas del servicio técnico', 'success')
        else:
            flash('No se encontraron facturas que necesiten migración', 'info')
        
        return redirect(url_for('servicio_tecnico'))
        
    except Exception as e:
        flash(f'Error migrando facturas: {str(e)}', 'danger')
        return redirect(url_for('servicio_tecnico'))
...
def reportes_servicio_tecnico():
    ...
    try:
        ...
        for orden in ordenes.values():
            if isinstance(orden, dict) and 'historial_estados' in orden:
                historial = orden['historial_estados']
                if len(historial) >= 2:
                    try:
                        fecha_inicio = parsear_fecha(historial[0]['fecha'])
                        fecha_fin = parsear_fecha(historial[-1]['fecha'])
                        
                        if not fecha_inicio or not fecha_fin:
                            continue
                        
                        # Calcular días de reparación
                        dias_reparacion = (fecha_fin - fecha_inicio).days
                        
                        # Filtrar tiempos anómalos (más de 90 días o negativos)
                        if 0 <= dias_reparacion <= 90:
                            tiempos_reparacion.append(dias_reparacion)
                            
                            # Agrupar por estado final
                            estado_final = historial[-1]['estado']
                            if estado_final not in tiempos_por_estado:
                                tiempos_por_estado[estado_final] = []
                            tiempos_por_estado[estado_final].append(dias_reparacion)
                            
                            # Agrupar por mes de inicio
                            mes_key = fecha_inicio.strftime('%Y-%m')
                            if mes_key not in tiempos_por_mes:
                                tiempos_por_mes[mes_key] = []
                            tiempos_por_mes[mes_key].append(dias_reparacion)
                            
                            # Agrupar por semana de inicio
                            semana_key = fecha_inicio.strftime('%Y-W%U')
                            if semana_key not in tiempos_por_semana:
                                tiempos_por_semana[semana_key] = []
                            tiempos_por_semana[semana_key].append(dias_reparacion)
                        
                    except (ValueError, KeyError) as e:
                        print(f"Error parseando fechas en orden {orden.get('id', 'desconocida')}: {e}")
                        continue
        
        # Estadísticas de tiempo mejoradas
        if tiempos_reparacion:
            # Ordenar tiempos para calcular percentiles
            tiempos_ordenados = sorted(tiempos_reparacion)
            n = len(tiempos_ordenados)
            
            tiempo_promedio = sum(tiempos_reparacion) / n
            tiempo_mediana = tiempos_ordenados[n // 2] if n % 2 == 1 else (tiempos_ordenados[n // 2 - 1] + tiempos_ordenados[n // 2]) / 2
            tiempo_minimo = min(tiempos_reparacion)
            tiempo_maximo = max(tiempos_reparacion)
            
            # Percentiles para análisis más detallado
            p25 = tiempos_ordenados[int(n * 0.25)] if n > 0 else 0
            p75 = tiempos_ordenados[int(n * 0.75)] if n > 0 else 0
            p90 = tiempos_ordenados[int(n * 0.90)] if n > 0 else 0
            
            # Calcular desviación estándar
            varianza = sum((x - tiempo_promedio) ** 2 for x in tiempos_reparacion) / n
            desviacion_estandar = varianza ** 0.5
            
        # Tiempos por mes (promedio) - Solo meses con datos
        tiempos_promedio_mes = {}
        for mes, tiempos in tiempos_por_mes.items():
            if tiempos:  # Solo incluir meses que tienen datos
                tiempos_promedio_mes[mes] = sum(tiempos) / len(tiempos)
        
        # Tiempos por semana (promedio) - Solo semanas con datos
        tiempos_promedio_semana = {}
        for semana, tiempos in tiempos_por_semana.items():
            if tiempos:  # Solo incluir semanas que tienen datos
                tiempos_promedio_semana[semana] = sum(tiempos) / len(tiempos)
        
        # Calcular tendencias avanzadas y análisis de patrones
        tendencias_mes = {}
        analisis_patrones = {}
        predicciones = {}
        
        if len(tiempos_promedio_mes) >= 2:
            meses_ordenados_tiempo = sorted(tiempos_promedio_mes.items())
            
            # Análisis de tendencias mensuales
            for i in range(1, len(meses_ordenados_tiempo)):
                mes_actual = meses_ordenados_tiempo[i][0]
                mes_anterior = meses_ordenados_tiempo[i-1][0]
                tiempo_actual = meses_ordenados_tiempo[i][1]
                tiempo_anterior = meses_ordenados_tiempo[i-1][1]
                
                if tiempo_anterior > 0:
                    cambio_porcentual = ((tiempo_actual - tiempo_anterior) / tiempo_anterior) * 100
                    tendencias_mes[mes_actual] = {
                        'tiempo': tiempo_actual,
                        'cambio': cambio_porcentual,
                        'tendencia': 'mejorando' if cambio_porcentual < 0 else 'empeorando' if cambio_porcentual > 0 else 'estable',
                        'velocidad_cambio': abs(cambio_porcentual),
                        'nivel_impacto': 'alto' if abs(cambio_porcentual) > 20 else 'medio' if abs(cambio_porcentual) > 10 else 'bajo'
                    }
            
            # Análisis de patrones estacionales
            if len(meses_ordenados_tiempo) >= 3:
                tiempos = [item[1] for item in meses_ordenados_tiempo]
                promedio_general = sum(tiempos) / len(tiempos)
                
                # Identificar meses pico y valle
                mes_pico = max(meses_ordenados_tiempo, key=lambda x: x[1])
                mes_valle = min(meses_ordenados_tiempo, key=lambda x: x[1])
                
                analisis_patrones = {
                    'promedio_general': promedio_general,
                    'mes_pico': {'mes': mes_pico[0], 'tiempo': mes_pico[1]},
                    'mes_valle': {'mes': mes_valle[0], 'tiempo': mes_valle[1]},
                    'variabilidad': max(tiempos) - min(tiempos),
                    'tendencia_general': 'mejorando' if tiempos[-1] < tiempos[0] else 'empeorando' if tiempos[-1] > tiempos[0] else 'estable',
                    'consistencia': 'alta' if max(tiempos) - min(tiempos) < 2 else 'media' if max(tiempos) - min(tiempos) < 5 else 'baja'
                }
                ...
                if len(tiempos) >= 3:
                    ...
                    ultimos_3 = tiempos[-3:]
                    if len(ultimos_3) == 3:
                        tendencia_lineal = (ultimos_3[2] - ultimos_3[0]) / 2
                        prediccion_proximo = ultimos_3[2] + tendencia_lineal
                        
                        predicciones = {
                            'proximo_mes_estimado': max(0, prediccion_proximo),
                            'confianza': 'alta' if abs(tendencia_lineal) < 1 else 'media' if abs(tendencia_lineal) < 3 else 'baja',
                            'recomendacion': 'optimizar_procesos' if prediccion_proximo > promedio_general + 2 else 'mantener_ritmo' if abs(prediccion_proximo - promedio_general) < 2 else 'capitalizar_eficiencia'
                        }
        ...
        for estado, tiempos in tiempos_por_estado.items():
            ...
            tiempo_promedio = tiempo_mediana = tiempo_minimo = tiempo_maximo = 0
            p25 = p75 = p90 = desviacion_estandar = 0
            tiempos_promedio_mes = {}
            tiempos_promedio_semana = {}
        ...
        return render_template('servicio_tecnico/reportes.html',
                             ...
                             tiempos_promedio_mes=tiempos_promedio_mes,
                             tiempos_promedio_semana=tiempos_promedio_semana,
                             tendencias_mes=tendencias_mes,
...
# Rutas de respaldo que redirigen a las funciones adaptadas
@app.route('/notas-entrega/nueva', methods=['GET', 'POST'])
@login_required
def nueva_nota_entrega():
    """Redirige a la creación de nueva nota de entrega."""
    return redirect(url_for('nueva_factura'))

@app.route('/notas-entrega/<id>/editar', methods=['GET', 'POST'])
@login_required
def editar_nota_entrega(id):
    """Redirige a la edición de nota de entrega."""
    return redirect(url_for('editar_factura', id=id))

@app.route('/notas-entrega/<id>')
@login_required
def ver_nota_entrega(id):
    """Redirige a la vista de nota de entrega."""
    return redirect(url_for('ver_factura', id=id))

@app.route('/notas-entrega/<id>/imprimir')
@login_required
def imprimir_nota_entrega(id):
    """Redirige a la impresión de nota de entrega."""
    return redirect(url_for('imprimir_factura', id=id))

@app.route('/notas-entrega/nueva-original', methods=['GET', 'POST'])
@login_required
def nueva_nota_entrega_original():
...
# Rutas duplicadas eliminadas - usar las rutas de respaldo definidas arriba
